...
// CIDR reports strings that are not a CIDR prefix, of either family.
//
// It accepts whatever [net.ParseCIDR] accepts, which means host bits may be set:
// "10.0.0.5/24" passes here. [CIDRv4] and [CIDRv6] are stricter on that
// point as well as pinning the family.
func CIDR() validate.Check[string] {
...
// CIDRv4 reports strings that are not an IPv4 network address in CIDR form.
//
...
// [net.ParseCIDR] is happy with it. [CIDR] is the loose version.
//
...
// is rejected by [CIDRv6].
func CIDRv4() validate.Check[string] {
	return func(s string) error {
...
// CIDRv6 reports strings that are not an IPv6 network address in CIDR form.
//
// The mirror of [CIDRv4]: the prefix must be IPv6, and it must name the
...
func CIDRv6() validate.Check[string] {
	return func(s string) error {
		ip, ipn, err := net.ParseCIDR(s)
		if err != nil || ip.To4() != nil || !ipn.IP.Equal(ip) {
...
// IP reports strings that are not an IP address, of either family.
//
// It accepts whatever [net.ParseIP] accepts, so both "10.0.0.1" and "2001:db8::1"
// pass. A prefix such as "10.0.0.0/8" does not; that is [CIDR]'s job.
func IP() validate.Check[string] {
	return func(s string) error {
...
// IPv4 reports strings that are not an IPv4 address.
//
// An IPv4-mapped IPv6 address such as "::ffff:10.0.0.1" passes, because it is
// IPv4 in a different spelling. [IPv6] rejects the same string, so the two are
// complements over everything [IP] accepts.
func IPv4() validate.Check[string] {
	return func(s string) error {
...
// IPv6 reports strings that are not an IPv6 address.
//
// IPv4-mapped addresses such as "::ffff:10.0.0.1" are rejected here and accepted
// by [IPv4]; see there for why.
func IPv6() validate.Check[string] {
	return func(s string) error {
		if ip := net.ParseIP(s); ip == nil || ip.To4() != nil {
